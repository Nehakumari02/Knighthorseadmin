...
@section('content')
    ...
    <select name="assign_to" class="form--control nice-select">
        <option disabled selected value="">{{ __('Select Person') }}</option>
        
        @foreach($admins as $admin)
            <option value="{{ $admin->username }}" 
                {{ old('assign_to', $user->assign_to ?? '') == $admin->username ? 'selected' : '' }}>
                {{ $admin->username }}
            </option>
        @endforeach
...
<div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
    ...
    <select name="user_type" class="form--control nice-select">
        <option disabled selected value="">{{ __('Select User Type') }}</option>
        <option value="retailer" {{ old('user_type', $user->user_type ?? '') == 'retailer' ? 'selected' : '' }}>
            {{ __('Retailer') }}
        </option>
        <option value="wholesaler" {{ old('user_type', $user->user_type ?? '') == 'wholesaler' ? 'selected' : '' }}>
            {{ __('Wholesaler') }}
        </option>
    </select>
</div>
                    <div class="col-xl-3 col-lg-3 col-md-6 col-sm-6 form-group">
    <label for="verificationStatus">{{ __('Verification Status') }}</label>
    <select name="verificationStatus" id="verificationStatus" class="form--control nice-select">
        <option value="pending" {{ old('verificationStatus', $user->verificationStatus) == 'pending' ? 'selected' : '' }}>
            {{ __('Pending') }}
        </option>
        <option value="approved" {{ old('verificationStatus', $user->verificationStatus) == 'approved' ? 'selected' : '' }}>
            {{ __('Approved') }}
        </option>
        <option value="denied" {{ old('verificationStatus', $user->verificationStatus) == 'denied' ? 'selected' : '' }}>
            {{ __('Denied') }}
        </option>
    </select>
</div>
